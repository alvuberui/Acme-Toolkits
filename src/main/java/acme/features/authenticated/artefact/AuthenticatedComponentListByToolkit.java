...
@Service
public class AuthenticatedComponentListByToolkit implements AbstractListService<Authenticated, Artefact>{
	...
	// Internal state ---------------------------------------------------------
		...
		public Collection<Artefact> findMany(final Request<Artefact> request) {
			...
			result = this.repository.findComponentsByToolkitId(toolkitId);
