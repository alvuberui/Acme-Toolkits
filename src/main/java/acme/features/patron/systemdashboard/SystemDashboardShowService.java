...
public class SystemDashboardShowService implements AbstractShowService<Patron, SystemDashboard>{
	...
	public SystemDashboard findOne(final Request<SystemDashboard> request) {
		...
		if(this.repository.allArtefacts() == 0) {
			result.setRatioArtefactWithChimpum(0.0);
		}else {			
			result.setRatioArtefactWithChimpum(Double.valueOf(this.repository.allArtefactsWithChimpum())/this.repository.allArtefacts());
		}
	...
	public void unbind(final Request<SystemDashboard> request, final SystemDashboard entity, final Model model) {
		...
		request.unbind(entity, model, "ratioArtefactWithChimpum", "avgBudgetByCurrency", "deviationBudgetByCurrency",
